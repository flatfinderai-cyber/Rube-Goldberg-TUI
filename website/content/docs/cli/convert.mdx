...
```bash
rube-goldberg-tui convert [options] <prd-file>
```

The `convert` command transforms a PRD markdown file into a task format that rube-goldberg can execute. It extracts user stories, acceptance criteria, and quality gates to create structured task files.
...
rube-goldberg-tui convert ./tasks/prd-my-feature.md

# Specify output path
rube-goldberg-tui convert ./tasks/prd-my-feature.md --output ./my-tasks.json
```
...
rube-goldberg-tui convert --to json ./tasks/prd-my-feature.md
```
...
rube-goldberg-tui convert --force ./tasks/prd-my-feature.md
```
...
rube-goldberg-tui convert --branch feat/my-feature ./tasks/prd-my-feature.md
```
...
<Steps>
  ...
  <Step title="Generate Output">
    Creates the task file in the specified format with proper structure for rube-goldberg to execute.
  </Step>
</Steps>
...
<Callout type="info">
The `passes: false` field indicates the task hasn't been completed. rube-goldberg sets this to `true` when the task is done.
</Callout>
...
<Steps>
  ...
  <Step title="Convert to Tasks">
    ```bash
    rube-goldberg-tui convert ./tasks/prd-auth.md
    ```
  </Step>
...
```bash
rube-goldberg-tui convert --force ./tasks/prd-auth.md
# or
rube-goldberg-tui convert ./tasks/prd-auth.md --output ./new-tasks.json
```
