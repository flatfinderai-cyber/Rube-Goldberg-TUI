---
title: status
description: Check the current rube-goldberg TUI session status for monitoring and CI integration.
---

## Synopsis

```bash
rube-goldberg-tui status [options]
```
...
rube-goldberg-tui status --cwd /path/to/project
```
...
```bash
#!/bin/bash
# Check if rube-goldberg is done
status=$(rube-goldberg-tui status --json | jq -r '.status')
...
```yaml
- name: Check rube-goldberg Status
  run: |
    status=$(rube-goldberg-tui status --json | jq -r '.status')
    completed=$(rube-goldberg-tui status --json | jq -r '.tasks.completed')
    total=$(rube-goldberg-tui status --json | jq -r '.tasks.total')
    echo "rube-goldberg: $completed/$total tasks completed (status: $status)"
```
...
The status command looks for `.rube-goldberg-tui-session.json` in:
