---
title: Common Issues
description: FAQ-style solutions for common rube-goldberg TUI problems including task selection, agent errors, sessions, and rate limiting.
---
...
This error occurs when rube-goldberg TUI cannot find any tasks to execute.
...
If rube-goldberg TUI crashed or was forcefully terminated, tasks may remain stuck in the `in_progress` state.
...
rube-goldberg TUI selects tasks based on priority and dependency order. If an unexpected task is selected:
...
rube-goldberg TUI cannot locate your AI agent CLI executable.
...
4. Specify the agent explicitly:
   ```bash
   rube-goldberg-tui run --agent claude --prd ./prd.json
   ```
...
1. **Check subagent tracing settings**: High filtering may hide output
   ```toml
   # In .rube-goldberg-tui/config.toml
...
Another rube-goldberg TUI instance may be running, or a previous session didn't shut down cleanly.
...
4. **Manually remove the lock file:**
   ```bash
   rm .rube-goldberg-tui-session.json
   ```
...
If `rube-goldberg-tui resume` fails:

1. **Check session file exists:**
   ```bash
   ls -la .rube-goldberg-tui-session.json
   ```
...
rube-goldberg TUI saves progress after each iteration. If progress was lost:

1. **Check iteration logs** for completed work:
   ```bash
   rube-goldberg-tui logs --iteration 1
   ```
...
<Callout type="info">
Rate limiting is common with high-volume usage. rube-goldberg TUI has built-in strategies to handle this gracefully.
</Callout>

**Configure rate limit handling:**

```toml
# .rube-goldberg-tui/config.toml
...
1. **Use iteration delays:**
   ```bash
   rube-goldberg-tui run --delay 5000 --prd ./prd.json
   ```

2. **Limit concurrent sessions** - Don't run multiple rube-goldberg TUI instances simultaneously

3. **Consider using a lower-tier model** for less critical tasks:
   ```bash
   rube-goldberg-tui run --model haiku --prd ./prd.json
   ```
...
rube-goldberg TUI detects completion when the agent outputs `<promise>COMPLETE</promise>`. If tasks aren't being marked complete:

1. **Verify the agent outputs the token:**
   ```bash
   rube-goldberg-tui logs --iteration 5 --verbose | grep -i "promise"
   ```
...
1. **Review the iteration logs:**
   ```bash
   rube-goldberg-tui logs --task US-005 --verbose
   ```
...
rube-goldberg TUI looks for configuration in this order:
1. CLI flags
2. Project config: `.rube-goldberg-tui/config.toml`
3. Global config: `~/.config/rube-goldberg-tui/config.toml`
...
1. **Check config file syntax:**
   ```bash
   cat .rube-goldberg-tui/config.toml
   ```
...
3. **Check GitHub Issues** for similar problems:
   [github.com/subsy/rube-goldberg-tui/issues](https://github.com/subsy/rube-goldberg-tui/issues)
