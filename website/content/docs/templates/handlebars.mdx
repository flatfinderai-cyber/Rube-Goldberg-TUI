...
description: Complete reference for Handlebars syntax and available template variables in rube-goldberg TUI.
---

## Handlebars Basics

rube-goldberg uses [Handlebars](https://handlebarsjs.com) for template rendering. Handlebars is a logic-less templating language that uses double curly braces for variable substitution.
...
rube-goldberg provides these variables for template rendering:
...
Extracted from task metadata or parsed from description. rube-goldberg formats array-based criteria as a markdown checklist:
...
The progress file is managed automatically by rube-goldberg and includes:
...
By default, Handlebars escapes HTML entities. rube-goldberg disables this (using `noEscape: true`), so your content renders as-is:

```handlebars
{{!-- These render the same in rube-goldberg --}}
{{taskDescription}}
{{{taskDescription}}}
```
...
Note: `labels` is already comma-joined by rube-goldberg. For full control, access `task.labels` array directly.
...
3. Use `rube-goldberg-tui template show` to see available data
