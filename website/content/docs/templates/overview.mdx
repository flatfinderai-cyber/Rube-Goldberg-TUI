...
description: Learn how rube-goldberg TUI uses Handlebars templates to build prompts for AI agents.
---

## What Are Prompt Templates?

Prompt templates are [Handlebars](https://handlebarsjs.com) templates that transform task data into structured prompts for AI agents. When rube-goldberg picks a task from your tracker, it uses a template to build the prompt that gets sent to Claude Code or OpenCode.
...
When rube-goldberg executes a task:

<Steps>
  <Step title="Load Template">
    rube-goldberg loads the appropriate template based on your tracker type (JSON, Beads, or custom).
  </Step>
...
rube-goldberg includes optimized templates for each tracker type:
...
rube-goldberg searches for templates in this order, using the first one found:
...
<Steps>
  <Step title="Custom Path">
    ...
    ```bash
    rube-goldberg-tui run --prompt ./my-template.hbs
    ```
  </Step>

  <Step title="User Config Directory">
    Templates in `~/.config/rube-goldberg-tui/`:
    - `prompt.md` for JSON tracker
    - `prompt-beads.md` for Beads trackers
  </Step>

  <Step title="Built-in Template">
    Bundled defaults that ship with rube-goldberg TUI.
  </Step>
</Steps>

## Viewing Your Current Template

To see which template rube-goldberg is using:
