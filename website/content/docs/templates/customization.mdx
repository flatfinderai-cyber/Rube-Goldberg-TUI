...
While rube-goldberg's built-in templates work for most projects, customization lets you:
...
```bash
rube-goldberg-tui template init
```

This creates `.rube-goldberg-tui-prompt.hbs` in your project root, pre-populated with the current template for your tracker type.

<Callout type="tip">
The `.hbs` extension stands for Handlebars, the templating engine rube-goldberg uses. You can also use `.md` or any other extension—rube-goldberg just reads the file content.
</Callout>
...
├── .rube-goldberg-tui/
│   └── config.toml
├── .rube-goldberg-tui-prompt.hbs    # Custom template here
└── ...
```
...
prompt_template = ".rube-goldberg-tui-prompt.hbs"
```

Or pass it via CLI:

```bash
rube-goldberg-tui run --prompt .rube-goldberg-tui-prompt.hbs
```
...
```bash
rube-goldberg-tui template init --global
```
...
rube-goldberg maintains a **progress file** (`.rube-goldberg-tui/progress.md`) that summarizes work completed across iterations. Use the `{{recentProgress}}` variable to include this summary in your prompts:
...
**Default behavior**: rube-goldberg's built-in OOTB templates now include `{{recentProgress}}` to help agents understand context without receiving full iteration history.

### How It Works

1. After each iteration, rube-goldberg extracts key information to `{{progressFile}}` (default: `.rube-goldberg-tui/progress.md`)
2. On subsequent iterations, the content is available as `{{recentProgress}}`
3. The summary includes completed tasks, key changes, and notes from previous work

<Callout type="info">
The progress file is distinct from iteration logs (`.rube-goldberg-tui/iterations/`). Use `{{recentProgress}}` for context and `rube-goldberg-tui logs` for detailed debugging.
</Callout>

### Customizing Progress Content

The progress file is managed by rube-goldberg's engine. To customize what gets included:
...
rube-goldberg-tui run --prd ./prd.json --iterations 1 --headless
```
