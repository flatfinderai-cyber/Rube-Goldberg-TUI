...
The JSON tracker is the simplest way to use rube-goldberg TUI. It reads tasks from a local `prd.json` file with no external dependencies - perfect for getting started quickly.

<Callout type="tip">
The JSON tracker is ideal for single-developer projects or when you want to try rube-goldberg TUI without installing additional tools.
</Callout>
...
<Steps>
  ...
  <Step title="Run rube-goldberg TUI">
    ```bash
  ...
  <Step title="Watch Progress">
    As tasks complete, rube-goldberg updates `passes: true` in your `prd.json`.
  </Step>
</Steps>
...
```bash
rube-goldberg-tui convert --to json ./tasks/my-feature-prd.md
```
...
rube-goldberg TUI won't select a task until all its dependencies have `passes: true`.
...
When rube-goldberg TUI runs with the JSON tracker:
...
<TabsRoot defaultValue="tasks">
  <TabsList>
    <TabsTrigger value="tasks">Wrong Field: tasks</TabsTrigger>
    <TabsTrigger value="status">Wrong Field: status</TabsTrigger>
    <TabsTrigger value="unsupported">Unsupported Fields</TabsTrigger>
  </TabsList>
...
rube-goldberg-tui convert --to json ./tasks/my-prd.md --force
```
...
Run `rube-goldberg-tui convert --to json` to regenerate.
