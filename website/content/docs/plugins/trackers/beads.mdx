...
<Steps>
  ...
  <Step title="Run rube-goldberg TUI">
    ...
    ```bash
    rube-goldberg-tui run --tracker beads --epic beads-abc123
    ```
  </Step>
</Steps>
...
| Beads Status | rube-goldberg TUI Status | Description |
|--------------|------------------|-------------|
...
rube-goldberg TUI calls `bd sync` automatically when you run.
...
```bash
# CLI
rube-goldberg-tui run --tracker beads --epic beads-xyz

# With label filter in config
[trackerOptions]
labels = "rube-goldberg,frontend"
```
...
When rube-goldberg TUI runs with the Beads tracker:
...
# Or convert manually:
rube-goldberg-tui convert --to beads ./tasks/my-prd.md
```
