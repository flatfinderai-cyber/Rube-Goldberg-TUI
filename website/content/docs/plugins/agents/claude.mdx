...
description: Integrate Anthropic's Claude Code CLI with rube-goldberg TUI for AI-assisted coding.
---

## Claude Code Agent

The Claude Code agent plugin integrates with Anthropic's `claude` CLI to execute AI coding tasks. It's the default agent for rube-goldberg TUI and provides the richest feature set.

<Callout type="tip">
Claude Code supports **subagent tracing** - rube-goldberg TUI can show nested tool calls (Read, Write, Task) in real-time as Claude works.
</Callout>
...
<Steps>
  <Step title="Run with Claude">
    ...
    ```bash
    rube-goldberg-tui run --prd ./prd.json --agent claude
    ```
  </Step>
  ...
  <Step title="Select a Model">
    ...
    ```bash
    rube-goldberg-tui run --prd ./prd.json --model opus
    ```
  </Step>
...
Claude Code emits structured JSONL when running with `--output-format stream-json`. rube-goldberg TUI parses this to display:
...
<Callout type="info">
Subagent tracing requires Claude's streaming JSON output. rube-goldberg TUI automatically adds the necessary flags (`--verbose --output-format stream-json`) when tracing is enabled.
</Callout>

## Autonomous Operation

By default, Claude Code prompts for permission before modifying files. rube-goldberg TUI runs with `--dangerously-skip-permissions` to enable fully autonomous operation.
...
Claude Code supports adding directory context via `--add-dir`. rube-goldberg TUI extracts directories from any file paths in your task and adds them automatically.

This helps Claude understand your project structure when working on related files.

## How It Works

When rube-goldberg TUI executes a task with Claude:
...
rube-goldberg TUI builds these arguments:
