...
description: Use OpenCode CLI with multiple AI providers in rube-goldberg TUI.
---
...
<Callout type="tip">
OpenCode lets you use any supported AI provider with rube-goldberg TUI, including local models via Ollama.
</Callout>
...
<Steps>
  <Step title="Run with OpenCode">
    ...
    ```bash
    rube-goldberg-tui run --prd ./prd.json --agent opencode
    ```
  </Step>
  ...
  <Step title="Select a Model">
    ...
    ```bash
    rube-goldberg-tui run --prd ./prd.json --agent opencode --model anthropic/claude-3-5-sonnet
    ```
  </Step>
</Steps>
...
<Callout type="info">
Model names are validated by the provider's API. Invalid model names result in errors from OpenCode, not rube-goldberg TUI.
</Callout>
...
Using non-default agent types may trigger warning messages in OpenCode output. rube-goldberg TUI filters these automatically, but you may see them if running OpenCode directly.
</Callout>

## File Context

OpenCode supports attaching files via the `--file` flag. rube-goldberg TUI extracts file paths from your task and adds them automatically:
...
When rube-goldberg TUI executes a task with OpenCode:
...
OpenCode emits metadata lines during execution (tool calls, progress indicators). rube-goldberg TUI filters these for cleaner output:
...
<Steps>
  ...
  <Step title="Configure rube-goldberg TUI">
    ```toml
    agent = "opencode"
