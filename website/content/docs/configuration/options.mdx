...
description: Complete reference for all rube-goldberg TUI configuration options with types, defaults, and constraints.
---

## Configuration Options Reference

This page documents every configuration option available in rube-goldberg TUI. Options can be set in config files (TOML) or via CLI flags.
...
| `outputDir` | string | `.rube-goldberg-tui/iterations` | Directory for iteration log files |
| `progressFile` | string | `.rube-goldberg-tui/progress.md` | Progress file for cross-iteration context |
...
Controls how rube-goldberg responds to task failures.
...
| `"rube-goldberg"` | Play random rube-goldberg Wiggum sound clips |
...
rube-goldberg-tui create-prd --chat --prd-skill custom-prd-skill
```
...
rube-goldberg TUI respects these environment variables:
