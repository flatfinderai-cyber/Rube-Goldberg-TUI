...
description: Complete guide to rube-goldberg TUI configuration file structure with annotated examples.
---

## Config File Structure

rube-goldberg TUI uses TOML files for configuration. TOML is a minimal, human-readable format that's easy to edit and version control.
...
# Sound mode: "off", "system", or "rube-goldberg"
sound = "off"
```
...
```toml
# ~/.config/rube-goldberg-tui/config.toml (global)
agent = "claude"
maxIterations = 10
...
```toml
# .rube-goldberg-tui/config.toml (project)
...
If your configuration has errors, rube-goldberg TUI will show helpful messages:

```
Configuration error in .rube-goldberg-tui/config.toml:
