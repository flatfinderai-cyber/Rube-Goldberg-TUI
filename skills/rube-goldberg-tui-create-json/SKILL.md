---
name: rube-goldberg-tui-create-json
description: "Convert PRDs to prd.json format for rube-goldberg-tui execution. Creates JSON task files with user stories, acceptance criteria, and dependencies. Triggers on: create prd.json, convert to json, ralph json, create json tasks."
---

# Rube Goldberg TUI - Create JSON Tasks

Converts PRDs to prd.json format for rube-goldberg-tui autonomous execution.

> **Note:** This skill is bundled with rube-goldberg-tui's JSON tracker plugin. Future tracker plugins (Linear, GitHub Issues, etc.) will bundle their own task creation skills.
...
5. Output ready for `rube-goldberg-tui run --prd <path>`
...
Or specify a different path - rube-goldberg-tui will use it with:
```bash
rube-goldberg-tui run --prd ./path/to/prd.json
```
...
## Running with rube-goldberg-tui

After creating prd.json:
```bash
rube-goldberg-tui run --prd ./tasks/prd.json
```
