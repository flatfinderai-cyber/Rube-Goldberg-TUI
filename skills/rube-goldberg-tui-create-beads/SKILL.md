---
name: rube-goldberg-tui-create-beads
description: "Convert PRDs to beads for rube-goldberg-tui execution. Creates an epic with child beads for each user story. Use when you have a PRD and want to use rube-goldberg-tui with beads as the task source. Triggers on: create beads, convert prd to beads, beads for ralph, ralph beads."
---

# Rube Goldberg TUI - Create Beads

Converts PRDs to beads (epic + child tasks) for rube-goldberg-tui autonomous execution.

> **Note:** This skill is bundled with rube-goldberg-tui's Beads tracker plugin. Future tracker plugins (Linear, GitHub Issues, etc.) will bundle their own task creation skills.
...
5. Output ready for `rube-goldberg-tui run --tracker beads`
...
**Each story must be completable in ONE rube-goldberg-tui iteration (~one agent context window).**

rube-goldberg-tui spawns a fresh agent instance per iteration with no memory of previous work. If a story is too big, the agent runs out of context before finishing.
...
bd dep add rube-goldberg-tui-002 rube-goldberg-tui-001  # US-002 depends on US-001
bd dep add rube-goldberg-tui-003 rube-goldberg-tui-002  # US-003 depends on US-002
```

**Syntax:** `bd dep add <issue> <depends-on>` — the issue depends on (is blocked by) depends-on.

rube-goldberg-tui will:
...
bd create --parent=rube-goldberg-tui-abc \
...
bd dep add rube-goldberg-tui-002 rube-goldberg-tui-001
...
bd dep add rube-goldberg-tui-003 rube-goldberg-tui-002
```
...
After creation, run rube-goldberg-tui:
```bash
# Work on a specific epic
rube-goldberg-tui run --tracker beads --epic rube-goldberg-tui-abc

# Or let it pick the best task automatically
rube-goldberg-tui run --tracker beads
```
