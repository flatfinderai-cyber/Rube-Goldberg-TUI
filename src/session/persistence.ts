...
/**
 * Session file path relative to cwd (inside .rube-goldberg-tui directory)
 */
const SESSION_FILE = '.rube-goldberg-tui/session.json';
 ...
 * Saved to .rube-goldberg-tui/session.json
 */
    ...
    if (validationError) {
      console.warn(
        `Invalid session file: ${validationError}. ` +
          'Delete .rube-goldberg-tui/session.json to start fresh.'
      );
      return null;
    }
