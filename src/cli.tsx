...
Examples:
  rube-goldberg-tui                              # Start execution (same as 'run')
  rube-goldberg-tui create-prd                   # Create a new PRD interactively
  rube-goldberg-tui create-prd --chat            # Create PRD with AI chat mode
  rube-goldberg-tui convert --to json ./prd.md   # Convert PRD to JSON
  rube-goldberg-tui run                          # Start execution with defaults
  rube-goldberg-tui run --epic myproject-epic    # Run with specific epic
  rube-goldberg-tui run --prd ./prd.json         # Run with PRD file
  rube-goldberg-tui resume                       # Resume interrupted session
  rube-goldberg-tui status                       # Check session status
  rube-goldberg-tui status --json                # JSON output for CI/scripts
  rube-goldberg-tui logs                         # List iteration logs
  rube-goldberg-tui logs --iteration 5           # View specific iteration
  rube-goldberg-tui logs --task US-005           # View logs for a task
  rube-goldberg-tui logs --clean --keep 10       # Clean up old logs
  rube-goldberg-tui plugins agents               # List agent plugins
  rube-goldberg-tui plugins trackers             # List tracker plugins
  rube-goldberg-tui template show                # Show current prompt template
  rube-goldberg-tui template init                # Create custom template
  rube-goldberg-tui docs                         # Open documentation in browser
  rube-goldberg-tui docs quickstart              # Open quick start guide
`);
}
...
async function handleSubcommand(args: string[]): Promise<boolean> {
  ...
  if (command === 'version' || command === '--version' || command === '-v') {
    ...
    console.log(`rube-goldberg-tui ${pkg.default.version}`);
    return true;
  }
