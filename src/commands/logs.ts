/**
 * ABOUTME: Logs command for rube-goldberg-tui.
...
export async function executeLogsCommand(args: string[]): Promise<void> {
  ...
  if (!hasLogs) {
    ...
    console.log('Run rube-goldberg-tui to generate logs.');
    console.log('');
    return;
  }
  ...
  console.log('    rube-goldberg-tui logs --iteration 5        View iteration 5');
  console.log('    rube-goldberg-tui logs --task US-005        View logs for task');
  console.log('    rube-goldberg-tui logs --clean --keep 10    Clean old logs');
  console.log('');
}
...
export function printLogsHelp(): void {
  console.log(`
rube-goldberg-tui logs - View and manage iteration output logs

Usage: rube-goldberg-tui logs [options]
...
Description:
  Lists iteration output logs saved during rube-goldberg-tui execution.
  Logs are stored in .rube-goldberg-tui/iterations/ and include:
...
Examples:
  rube-goldberg-tui logs                        # List all iteration logs
  rube-goldberg-tui logs --iteration 5          # View iteration 5 in detail
  rube-goldberg-tui logs -i 5                   # Shorthand for above
  rube-goldberg-tui logs --task US-005          # View all iterations for US-005
  rube-goldberg-tui logs -t US-005              # Shorthand for above
  rube-goldberg-tui logs --clean --keep 10      # Delete all but 10 most recent logs
  rube-goldberg-tui logs --clean --dry-run      # Preview cleanup without deleting
`);
}
