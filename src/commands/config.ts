...
function formatSourceInfo(source: ConfigSource): string {
  ...
  if (source.projectPath) {
    ...
    lines.push(`│   ○ .rube-goldberg-tui/config.toml (not found in project tree)`);
  }
...
Configuration Files:
  ...
  Project:  .rube-goldberg-tui/config.toml (in project root or any parent directory)
...
export async function executeConfigCommand(args: string[]): Promise<boolean> {
  ...
  console.log('Run "rube-goldberg-tui config help" for available commands');
  return true;
}
