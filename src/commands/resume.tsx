/**
 * ABOUTME: Resume command for rube-goldberg-tui.
...
async function runHeadless(
  ...
  engine.on((event) => {
    switch (event.type) {
      ...
      case 'engine:paused':
        console.log('\nPaused. Use "rube-goldberg-tui resume" to continue.');
        currentState = pauseSession(currentState);
        savePersistedSession(currentState).catch(() => {
          // Log but don't fail on save errors
        });
...
export async function executeResumeCommand(args: string[]): Promise<void> {
  ...
  if (!hasSession) {
    ...
    console.error('');
    console.error('Start a new session with: rube-goldberg-tui run');
    process.exit(1);
  }
  ...
  if (!isSessionResumable(persistedState)) {
    ...
    if (summary.status === 'completed') {
      console.error('Session has already completed. Start a new session with: rube-goldberg-tui run');
    } else {
      console.error('Session cannot be resumed. Start a new session with: rube-goldberg-tui run --force');
    }
    process.exit(1);
  }
  ...
  if (finalState.status === 'completed') {
    ...
    console.log('\nSession paused. Use "rube-goldberg-tui resume" to continue.');
  } else {
...
export function printResumeHelp(): void {
  console.log(`
rube-goldberg-tui resume - Continue from a previous session

Usage: rube-goldberg-tui resume [options]
...
Description:
  ...
  The session state is stored in .rube-goldberg-tui/session.json.
  ...
  Completed or failed sessions cannot be resumed. Use 'rube-goldberg-tui run --force'
  to start a new session.

Examples:
  rube-goldberg-tui resume              # Resume session in current directory
  rube-goldberg-tui resume --headless   # Resume without TUI
  rube-goldberg-tui resume --force      # Force resume (override stale lock)
`);
}
