/**
 * ABOUTME: Status command for rube-goldberg-tui (headless).
...
function printHumanStatus(
  ...
  if (!session && status === 'no-session') {
    ...
    console.log('');
    console.log('Start a new session with: rube-goldberg-tui run');
    return;
  }
  ...
  if (resumable) {
    ...
    console.log('');
    console.log('  To start new: rube-goldberg-tui run');
...
export function printStatusHelp(): void {
  console.log(`
rube-goldberg-tui status - Check session status (headless)

Usage: rube-goldberg-tui status [options]
...
Examples:
  rube-goldberg-tui status              # Human-readable output
  rube-goldberg-tui status --json       # JSON output for scripts
...
CI/Script Usage:
  ...
  # Get task progress
  rube-goldberg-tui status --json | jq '.session.progress.percent'
`);
}
