 ...
 * Provides rube-goldberg-tui template show and rube-goldberg-tui template init commands.
 */
...
function showTemplateHelp(): void {
  console.log(`
${BOLD}rube-goldberg-tui template${RESET} - Manage prompt templates

${BOLD}Commands:${RESET}
  ${CYAN}show${RESET}              Display the current template being used
  ${CYAN}init${RESET}              Copy default template for customization
  ${CYAN}init-prompts${RESET}      Initialize user prompt files in ~/.config/rube-goldberg-tui/
...
${BOLD}Examples:${RESET}
  rube-goldberg-tui template show                    # Show current template
  rube-goldberg-tui template show --tracker beads    # Show built-in beads template
  rube-goldberg-tui template init                    # Copy default template for customization
  rube-goldberg-tui template init --tracker beads    # Copy beads template
  rube-goldberg-tui template init-prompts            # Initialize ~/.config/rube-goldberg-tui/ with prompts

${BOLD}Prompt Files:${RESET}
  The --prompt option in 'rube-goldberg-tui run' searches for prompts in this order:
  1. Explicit --prompt <path> argument
  2. ~/.config/rube-goldberg-tui/prompt.md or prompt-beads.md (based on tracker mode)
...
async function handleInitTemplate(args: string[]): Promise<void> {
  ...
  console.log(`  2. Add to your ${CYAN}.rube-goldberg-tui/config.toml${RESET}:`);
  console.log(`     ${DIM}prompt_template: ${path.relative(cwd, outputPath)}${RESET}`);
  console.log(`\n${DIM}See 'rube-goldberg-tui template show' for available variables${RESET}`);
}

/**
 * Handle the 'template init-prompts' command.
 * Initializes user prompt files in ~/.config/rube-goldberg-tui/.
 */
function handleInitPrompts(args: string[]): void {
  ...
  if (result.success) {
    ...
    console.log(`${DIM}Use --prompt <path> in 'rube-goldberg-tui run' for one-off custom prompts.${RESET}`);
  } else {
