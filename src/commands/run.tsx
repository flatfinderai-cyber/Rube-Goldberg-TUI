/**
 * ABOUTME: Run command implementation for rube-goldberg-tui.
...
Examples:
  rube-goldberg-tui run                              # Start with defaults
  rube-goldberg-tui run --epic rube-goldberg-tui-45r         # Run with specific epic
  rube-goldberg-tui run --prd ./prd.json             # Run with PRD file
  rube-goldberg-tui run --agent claude --model opus  # Override agent settings
  rube-goldberg-tui run --tracker beads-bv           # Use beads-bv tracker
  rube-goldberg-tui run --iterations 20              # Limit to 20 iterations
  rube-goldberg-tui run --resume                     # Resume previous session
  rube-goldberg-tui run --no-tui                     # Run headless for CI/scripts
`);
}
...
async function promptResumeOrNew(cwd: string): Promise<'resume' | 'new' | 'abort'> {
  ...
  if (resumable) {
    ...
    console.log('');
    console.log('  To resume:  rube-goldberg-tui resume');
    console.log('  To start fresh: rube-goldberg-tui run --force');
    console.log('');
    console.log('Starting fresh session...');
    console.log('(Use --resume flag or "rube-goldberg-tui resume" command to continue)');
    return 'new';
  } else {
...
export async function executeRunCommand(args: string[]): Promise<void> {
  ...
  if (!configExists && !options.noSetup) {
    ...
    console.log('');
    console.log('No .rube-goldberg-tui/config.toml configuration found in this project.');
    console.log('');
    ...
    if (!result.success) {
      if (result.cancelled) {
        console.log('Run "rube-goldberg-tui setup" to configure later,');
        console.log('or use "rube-goldberg-tui run --no-setup" to skip setup.');
        return;
      }
  ...
  if (allComplete) {
    ...
    await savePersistedSession(persistedState);
    console.log('\nSession state saved. Use "rube-goldberg-tui resume" to continue.');
  }
