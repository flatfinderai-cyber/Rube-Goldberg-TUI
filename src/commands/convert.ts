/**
 * ABOUTME: Convert command for rube-goldberg-tui.
...
export function parseConvertArgs(args: string[]): ConvertArgs | null {
  ...
  if (!input) {
    ...
    console.log('Usage: rube-goldberg-tui convert --to json ./tasks/prd-feature.md');
    return null;
  }
...
export function printConvertHelp(): void {
  console.log(`
rube-goldberg-tui convert - Convert PRD markdown to JSON or Beads format

Usage: rube-goldberg-tui convert --to <format> <input-file> [options]
...
Description:
  ...
  For JSON format (--to json):
    Creates a prd.json file for use with \`rube-goldberg-tui run --prd ./prd.json\`
...
Examples:
  ...
  rube-goldberg-tui convert --to json ./tasks/prd-feature.md
  rube-goldberg-tui convert --to json ./docs/requirements.md -o ./custom.json

  # Convert to Beads format
  rube-goldberg-tui convert --to beads ./tasks/prd-feature.md
  rube-goldberg-tui convert --to beads ./prd.md --labels "frontend,sprint-1"
`);
}
...
async function executeJsonConversion(
  ...
  if (!branchName) {
    ...
    branchName = await promptText('Git branch name for this work:', {
      ...
      required: true,
      help: 'The git branch that will be used when running rube-goldberg-tui',
    });
  }
  ...
  console.log();
  printInfo(`Run with: rube-goldberg-tui run --prd ${outputPath}`);
}
...
async function executeBeadsConversion(
  ...
  console.log();
  printInfo(`Run with: rube-goldberg-tui run --epic ${result.epicId}`);
}
