/**
 * ABOUTME: Create-PRD command for rube-goldberg-tui.
...
export function printCreatePrdHelp(): void {
  console.log(`
rube-goldberg-tui create-prd - Create a new PRD with AI assistance

Usage: rube-goldberg-tui create-prd [options]
       rube-goldberg-tui prime [options]
...
Description:
  ...
  The AI agent (using the rube-goldberg-tui-prd skill):
  ...
  Requires an AI agent to be configured. Run 'rube-goldberg-tui setup' to configure one.

Examples:
  rube-goldberg-tui create-prd                      # Start AI-powered PRD creation
  rube-goldberg-tui prime                           # Alias for create-prd
  rube-goldberg-tui create-prd --agent claude       # Use specific agent
  rube-goldberg-tui create-prd --output ./docs      # Save PRD to custom directory
`);
}
...
async function runChatMode(parsedArgs: CreatePrdArgs): Promise<PrdCreationResult | null> {
  ...
  if (!agent) {
    ...
    console.error('  1. Run "rube-goldberg-tui setup" to configure an agent');
 ...
 * If a tracker format is selected, launches rube-goldberg-tui run with the tasks loaded.
 */
export async function executeCreatePrdCommand(args: string[]): Promise<void> {
  ...
  await requireSetup(cwd, 'rube-goldberg-tui prime');
  ...
  if (parsedArgs.prdSkill) {
    if (!storedConfig.skills_dir?.trim()) {
      ...
      console.error('Set skills_dir in ~/.config/rube-goldberg-tui/config.toml or .rube-goldberg-tui/config.toml.');
      process.exit(1);
    }
