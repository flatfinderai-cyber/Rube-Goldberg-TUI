/**
 * ABOUTME: Plugin registry for tracker plugins.
 * Handles discovery, registration, and lifecycle management of tracker plugins.
...
/**
 * Default path for user-installed tracker plugins
 */
...
/**
 * Registry for tracker plugins.
 * Singleton pattern - use TrackerRegistry.getInstance() to access.
 */
export class TrackerRegistry {
   ...
   */
  getPluginMeta(pluginId: string): TrackerPluginMeta | undefined {
   ...
   */
  createInstance(pluginId: string): TrackerPlugin | undefined {
  ...
  async initialize(): Promise<PluginLoadResult[]> {
    ...
    // Built-in plugins are registered via registerBuiltinPlugins()
