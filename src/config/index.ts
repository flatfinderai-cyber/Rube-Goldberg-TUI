...
/**
 * Global config file path (~/.config/rube-goldberg-tui/config.toml)
 */
const GLOBAL_CONFIG_PATH = join(homedir(), '.config', 'rube-goldberg-tui', 'config.toml');

/**
 * Project config directory name (.rube-goldberg-tui in project root)
 */
const PROJECT_CONFIG_DIR = '.rube-goldberg-tui';

/**
 * Project config file name (config.toml inside .rube-goldberg-tui directory)
 */
 ...
 * Looks for .rube-goldberg-tui/config.toml in each directory up to root.
 ...
 * Project config (.rube-goldberg-tui/config.toml) overrides global config (~/.config/rube-goldberg-tui/config.toml).
 ...
 * Loads both global (~/.config/rube-goldberg-tui/config.toml) and project (.rube-goldberg-tui/config.toml) configs.
 */
export async function buildConfig(
  ...
  // This allows `rube-goldberg-tui run --prd ./prd.json` to work without needing `--tracker json`
...
/**
 * Save configuration to the project config file (.rube-goldberg-tui/config.toml).
...
export async function saveProjectConfig(
  ...
  // Ensure the .rube-goldberg-tui directory exists
...
/**
 * Check if rube-goldberg-tui setup has been completed.
...
export async function checkSetupStatus(
  ...
  if (!configExists) {
    return {
      ...
      configPath: null,
      message: 'No configuration found. Run "rube-goldberg-tui setup" to configure.',
    };
  }
  ...
  if (!agentConfigured) {
    return {
      ...
      agentConfigured: false,
      configPath,
      message: 'No agent configured. Run "rube-goldberg-tui setup" to configure an agent.',
    };
  }
...
export async function requireSetup(
  ...
  if (!status.ready) {
    console.error('');
    console.error(`${commandName} requires rube-goldberg-tui to be configured.`);
    console.error('');
    if (status.message) {
    ...
    console.error('  rube-goldberg-tui setup');
    console.error('');
    process.exit(1);
  }
}
